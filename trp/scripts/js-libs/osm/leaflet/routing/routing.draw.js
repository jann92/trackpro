...
L.Routing.Draw = L.Handler.extend({
  ...
  , _onMouseClick: function (e) {
      if (this._hidden) { return; }
          count = this._parent.getAllMarkers().length + 1;

      var label = "" + count + "";

      var marker, latlng, last;

      var span = "<span class='ruler-marker-label'>" + label + "</span>";
      
      var icon = L.divIcon({ html: "<img src='../../contents/images/osm/circle-green.png' style='width:30px;height:30px;'>"+ span +"</img>", className: 'ruler-marker-label' });
      ...
      marker = new L.Marker(latlng, { icon: icon, title: this.options.tooltips.waypoint });
