...
namespace Philgps_WebAPI.Controllers.ObjectDelivery
{
    ...
    {
        //TFDBEntities tfdb = new TFDBEntities();
        ...
        //    var uservalue = userid.GetUserRoleValue(config_name.GetDBConfig("TFDB"));
        //    var ufunc = userid.GetUserRoleValue(config_name.GetDBConfig("TFDB")).GetUserFunctions(config_name.GetDBConfig("TFDB"));
        ...
        //    if (ufunc.Where(uf => uf.functionName == "Delivery" && uf.isOn == true).Count() == 0)
        //    {
        //        return false;
        //    }
        //    else
        ...
        public IHttpActionResult CreateDelivery(po_DeliveryDetails deliveryDetails)
        {
            ...
            //if (groupcodeview)
            //{
            //    return BadRequest("You are not allowed to access this API.");
            //}

            //string[] userlist = DHLUsers.ToString().Split(new char[] { ',' });

            using (TFDBEntities tfdb = new TFDBEntities(WebConfigurationName))
            {
                //var username = Claims.Where(x => x.Type == "user_name").FirstOrDefault().Value.ToString();
                //var userid = Claims.Where(x => x.Type == "user_id").FirstOrDefault().Value.ToString().strToInt();
                ...
                if (existDN.Count() > 0)
                {
                    ...
                    if (od.cancelled == true)
                    {
                        ...
                        od.remarks = null;
                        //od.remarks = (userlist.Any(x => x == username.ToString() && x.Length == username.Length) == true) ? "DHL" : null;
                        od.enabled = false;
        ...
        public IHttpActionResult StartDelivery(po_Trigger deliveryTrigger)
        {
            ...
            using (TFDBEntities tfdb = new TFDBEntities(WebConfigurationName))
            {
                ...
                odf.start_location = deliveryTrigger.location;
                odf.enabled = true;
                tfdb.SaveChanges();
        ...
        public IHttpActionResult CompleteDelivery(po_Trigger deliveryTrigger)
        {
            ...
            using (TFDBEntities tfdb = new TFDBEntities(WebConfigurationName))
            {
                ...
                odf.end_location = deliveryTrigger.location;
                odf.enabled = false;
                tfdb.SaveChanges();
            ...
            using (TFDBEntities tfdb = new TFDBEntities(WebConfigurationName))
            {
                ...
                var dfl = (from n in tfdb.PGPS_ObjectDelivery.ToList()
                           ...
                           select new DeliveryDetails()
                           {
                               ...
                               end_datetime = (n.end_datetime == null) ? DateTime.Now.ToString("yyyy-MM-dd HH:mm") : n.end_datetime.Value.ToString("yyyy-MM-dd HH:mm"),
                               start_datetime = (n.start_datetime == null) ? DateTime.Now.ToString("yyyy-MM-dd HH:mm") : n.start_datetime.Value.ToString("yyyy-MM-dd HH:mm"),
                               assetID = n.object_id
                           }).ToList();
        ...
        public IHttpActionResult DeliveryCurrentList(string _objectID)
        {
            ...
            using (TFDBEntities tfdb = new TFDBEntities(WebConfigurationName))
            {
                ...
                // var dfl = tfdb.PGPS_ObjectDelivery.Where(x => x.object_id == objectID).ToList();
